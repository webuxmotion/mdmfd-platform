                ...
                <div class="sixteen wide column">
                    <table class="ui compact celled definition table">
                        ...
                        <?php foreach($plugins as $directory => $plugin): ?>
                            <?php
                              if (isset($plugin['is_active'])) {
                                $is_active = $plugin['is_active'];
                              } else {
                                $is_active = 0;
                              }

                              if (isset($plugin['is_install'])) {
                            ...
                            <tr class="top aligned">
                                <td class="collapsing">
                                    <div class="ui fitted toggle checkbox">
                                        <input type="checkbox" data-active="<?=$is_active?>" onchange="plugin.activate(this, <?php echo $plugin['plugin_id'] ?>)"<?php if ($is_active) echo ' checked'; ?>>
                                        <label></label>
                                    </div>
                                </td>
                                <td>
